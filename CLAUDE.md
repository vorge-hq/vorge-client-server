...
## Current focus
0. **P3.5 — Word/PDF export** ⬅ **server-side DONE 2026-07-04 (252 unit / 124 integration green).** `GET /api/assessments/:id/export?format=docx|pdf` — Word via `docx`, PDF via `pdfkit` (chosen over headless print-CSS: DoD PDF checks are minimal, pdfkit is fast/CI-safe). `exportService.js` (one `deriveTables` → docx+pdf, cover + Document Approvals/Version Control front-matter + Sections 1–8 + Appendices + watermark) + `exportRepository.js` (frozen `versions` snapshot when Approved; approvals from lead-author + audit sign-offs). Export role = all section-readers; Mitigation Owner → 403. Watermark on any non-Approved state; `export` audit row per download. Full §P3.5 DoD in `tests/integration/export.test.js` (15 cases). **Remaining P3.5 follow-on:** wire the client workspace Export button (prod → endpoint download; demo stub) — no DoD spec'd yet, BACKLOG. Next phase proper = **P4 (AI module)**.
