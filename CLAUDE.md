...
## Current focus
- **Security (2026-07-10, awaiting review):** AUDIT-BRIEF hardening on branch `security/audit-hardening-2026-07-10` — auth rate limiting, helmet, token-claim acting role, RLS boot guard, MFA recovery lockout, admin-reset gate, dep bumps (386 unit / 202 client green; integration not re-run — no local DB). DB TLS CA-pinning + Redis caches deliberately left for P5. Not merged/pushed; deferred gaps in roadmap "Security audit follow-ups". Resume P4 O6 after this lands.
0. **P4 + P4.5 — building. O1 ✅ + O2 ✅ + F2 gate ✅ + O3 ✅ (semantic search) + O4 ✅ (smart tagging) + O5 ✅ (drafted summary) — all 2026-07-04. ⬅ NEXT: Opus build session O6 (anomaly detection server engine, AD-2+ — `/model opus`).** Model split = Fable specs/gates, Opus builds. **O5 delivered drafted Executive Summary/Conclusion** (`POST /api/assessments/:id/sections/:n/generate-draft`, n∈{1,8}; §9.1 gate Author AND state != Approved → 403/409, `rejectMitigationOwner`; gathers the Sections 2–7 bundle → `buildPromptContext` scope invariant → `runAiCall({kind:'text'})` with `ai/prompts/draftedSummary.js`, risk distribution DERIVED from stored `{consequence,likelihood}`; **AI original retained** in `ai-draft-generated` audit `metadata.draftText` — no section write, no lock bump — beside the edited final saved via the §P3 section path; "Draft with AI" flow moved into §1/§8 editors, `AIDraftModal` presentational through the prod↔demo seam; 386 unit / 172 integration / 202 client). No gate before O6 (next gate is F3 after O8). **Binding playbook: `docs/plans/p4-execution-plan.md`** — session order O1–O9, architecture (`server/src/ai/` module, `runAiCall` contract), and the escalation rule (deviations → Open questions section, never improvised). **F2 verified the O2 foundation and resolved all 3 open questions (now binding, in the playbook's Open questions + O7 block):** retry taxonomy (permanent 4xx/validation → no retry, `502 AI_CALL_FAILED`; classifier at the gateway seam), scope guards (`AI_SCOPE_MISSING`; `consistency_flagging` is the only operator-scoped gated feature — its entitlement is enforced by the O7 job's entitled-facility selection), and the O7 job's elevated DB-connection requirement for operator-scoped rows. DoD: `docs/test-specs.md` §P4 (extended at F2) + **§P4.5**. Remaining gate: **F3 after O8** (PLATFORM_OWNER + provisioning) — STOP and ask the user to switch models. Decisions standing: entitlements owner-only v1 (keys `anomaly_detection`/`consistency_flagging`/`offline_mode`); P4.5 support access = **link-only** (no impersonation). Build sessions: `/model opus`, start by reading the playbook.
