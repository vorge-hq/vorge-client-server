...
## Current focus
- **Security (2026-07-10, awaiting review):** AUDIT-BRIEF hardening on branch `security/audit-hardening-2026-07-10` — auth rate limiting, helmet, token-claim acting role, RLS boot guard, MFA recovery lockout, admin-reset gate, dep bumps. DB TLS CA-pinning + Redis caches left for P5. Not merged/pushed; deferred gaps in roadmap "Security audit follow-ups". Prefer merge before/with O6.
0. **P4 + P4.5 — building. O1–O6 ✅ + F2 ✅ + F-AI ✅ (2026-07-16). ⬅ NEXT: Opus O7 (cross-facility consistency flagging, nightly batch — `/model opus`).** Binding playbook: `docs/plans/p4-execution-plan.md` (gates: F2 ✅ · F-AI ✅ · F3 after O8). After O7: O8 → **F3** → O9. O6 is server-only: AD-1's client chip/ack still computes locally (wiring is a roadmap backlog item). DoD: `docs/test-specs.md` §P4 + §P4.5.
