...
## Current focus
0. **P4 + P4.5 — SPECCED 2026-07-04, build NOT started. ⬅ NEXT: Opus build session O1 (library CRUD).** All design work was front-loaded in a Fable session (docs-only; model split = Fable specs/gates, Opus builds). **Binding playbook: `docs/plans/p4-execution-plan.md`** — session order O1–O9, architecture (`server/src/ai/` module, `ai_call_log`/`ai_budgets`/`facility_entitlements` migrations, `runAiCall` contract), and the escalation rule (deviations → Open questions section, never improvised). DoD: `docs/test-specs.md` §P4 (existing) + **§P4.5 (authored 2026-07-04)**. Two mandatory Fable review gates: **F2 after O2** (AI foundation) and **F3 after O8** (PLATFORM_OWNER + provisioning) — STOP at each and ask the user to switch models. Decisions resolved 2026-07-04: entitlements owner-only v1 (keys `anomaly_detection`/`consistency_flagging`/`offline_mode`); P4.5 support access = **link-only** (no impersonation). Build sessions: `/model opus`, start by reading the playbook.
