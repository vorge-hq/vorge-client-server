...
## Current focus
0. **P4 + P4.5 — building. O1 ✅ + O2 ✅ DONE 2026-07-04. ⬅ NEXT: F2 Fable review gate (STOP — switch to `/model fable`), then O3 (semantic search).** All design work was front-loaded in a Fable session (docs-only; model split = Fable specs/gates, Opus builds). **Binding playbook: `docs/plans/p4-execution-plan.md`** — session order O1–O9, architecture (`server/src/ai/` module, `ai_call_log`/`ai_budgets`/`facility_entitlements` migrations, `runAiCall` contract), and the escalation rule (deviations → Open questions section, never improvised). **O2 delivered the AI foundation** (`server/src/ai/*`, `aiBudgetService`, 3 RLS migrations, read-time entitlement gating, `aiImportBoundary` scan; 331 unit / 143 integration). **One Open question is logged for F2** (consistency_flagging entitlement at operator scope). DoD: `docs/test-specs.md` §P4 + **§P4.5**. Remaining gate: **F3 after O8** (PLATFORM_OWNER + provisioning) — STOP and ask the user to switch models. Decisions resolved 2026-07-04: entitlements owner-only v1 (keys `anomaly_detection`/`consistency_flagging`/`offline_mode`); P4.5 support access = **link-only** (no impersonation). Build sessions: `/model opus`, start by reading the playbook. **F2 gate: `/model fable`, review ONLY the O2 diff against the F2 checklist in the playbook.**
