...
## Current focus
0. **P3.5 — Word/PDF export ✅ COMPLETE 2026-07-04 (261 unit / 184 client / 124 integration green).** Server: `GET /api/assessments/:id/export?format=docx|pdf` — Word via `docx`, PDF via `pdfkit` (chosen over headless print-CSS: DoD PDF checks are minimal, pdfkit is fast/CI-safe). `exportService.js` (one `deriveTables` → docx+pdf, cover + Document Approvals/Version Control front-matter + Sections 1–8 + Appendices + watermark) + `exportRepository.js` (frozen `versions` snapshot when Approved; approvals from lead-author + audit sign-offs). Export role = all section-readers; Mitigation Owner → 403. Watermark on any non-Approved state; `export` audit row per download. §P3.5 DoD in `tests/integration/export.test.js` (15 cases) + `services/exportService.test.js` (coverage gate). Client: workspace Tools → "Export document" `ExportModal` (Word/PDF, watermark note); `WorkspaceContext.exportDocument` prod↔demo seam (prod blob download via `apiDownload`; demo no-fetch); RTL `modals/ExportModal.test.jsx`. Only Phase-3 custom-template upload deferred (§16.5, BACKLOG). **⬅ NEXT PHASE = P4 (AI module) — NOT started; awaiting user go-ahead.**
1. **P3 — Write/section API ✅ COMPLETE (2026-07-04).** Server + client flip both done (250 unit / 176 client / 109 integration at close). Full history below for reference.
