...
- The read adapters live in `client/src/api/adapters.js` (`toClientAsset`/`toClientThreat`/`toClientEvaluation`/`toClientLinks`). The **write** adapter for assets (`toServerAssetPayload`) packs using the inverse of this table and **landed 2026-07-04** for Section 3 (add/update/remove through the prod↔demo seam in `WorkspaceContext`, blur-persist, lockVersion sync, 409 reload). Threats/links/evaluations/contributors write adapters are the same-mold follow-on (not yet built).
