...
- The read adapters live in `client/src/api/adapters.js` (`toClientAsset`/`toClientThreat`/`toClientEvaluation`/`toClientLinks`). The **write** adapters pack using the inverse of this table and **all landed 2026-07-04**: `toServerAssetPayload` (assets), `toServerThreatPayload` (threats), `toServerEvaluationPayload` (evaluations); links use `toggleMatrix`→`PUT /links` (enable/disable, not delete); contributors use a whole-list `PUT` (identity records). All wired through the prod↔demo seam in `WorkspaceContext` (local-optimistic per keystroke + blur/discrete persist, `stateRef` for fresh lockVersion, 409 reload). **Evaluation create (was a server gap) — RESOLVED 2026-07-04:** evaluations still have no standalone create endpoint, but `setAssetThreatLink` now AUTO-CREATES an empty evaluation when a pair is enabled and echoes it in the `PUT /links` response, so the client gets a real UUID row to PATCH the moment a Section-6 cell is scoped (mirrors the demo flow). Disable never deletes it. `persistEvaluation`'s UUID guard remains as belt-and-suspenders for any client-only stub that predates hydration.
