...
- **Boot guards**: server refuses to start if `MFA_ENCRYPTION_KEY` is wrong length or invalid base64. **In production**, server also refuses to start if the key is the dev placeholder. In dev/test, a 32-zero-byte placeholder is accepted so the container boots without manual env setup — operationally permissive in dev, strict in prod (mirrors how `JWT_SECRET` works). **Production deploy requires generating a real key:** `openssl rand -base64 32`. Additionally, server refuses to start if `NODE_ENV === 'production'` AND `__MFA_TEST_MODE__ === '1'`.
