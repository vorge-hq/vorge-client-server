...
**Created**: 2026-05-26
**Purpose**: For each of the 5 chunk-4 screens, decide upfront whether it needs designer input before build, or whether Claude ships in the default Vorge style. Per the brief: default to "Claude's judgment, ship in default style" for chunk 4 unless the screen has obvious UX risk.

The Vorge default style (established in `LoginPage.jsx`, `ForgotPasswordPage.jsx`, `ResetPasswordPage.jsx` from chunks 0–3):
- Centered `max-w-[400px]` card on `bg-surface-sunken`.
- Vorge brand header (Shield icon + "Vorge SRA Platform" tag).
...
|---|---|---|---|---|
| 1 | **Enrollment** | `client/src/pages/auth/MfaEnrollPage.jsx` | **Claude's judgment, ship in default style** | Standard QR + manual-key + verify-code flow. Vorge style accommodates it directly. Only non-trivial detail: the recovery-code one-time display needs a clear "DOWNLOAD .TXT" affordance + an explicit "you will NOT see these again" warning before the user can dismiss. Will draft inline. |
...
- The Vorge form-card pattern is established and well-tested through chunks 0–3.
