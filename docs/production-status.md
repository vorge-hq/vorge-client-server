...
Last updated: 2026-07-16 · Branch: `main` (security/audit-hardening-2026-07-10 ✅ merged at 6bf33cf; no open branches). Brand: **Vorge** (rebranded from Vantage on 2026-06-02 — see SESSION_LOG). **F-AI ✅ passed 2026-07-16** — O6 (anomaly server engine) + O7 (cross-facility consistency) ✅ landed 2026-07-16; next P4 build is O8 (Platform Owner + provisioning API) → then the **F3 Fable gate**.

> **2026-07-10 — security hardening pass (✅ merged to main at 6bf33cf, pre-O6):** the AUDIT-BRIEF 5-agent review found the core posture strong (no SQLi, IDOR-proof tenancy, sound state machine/lockVersion, correct AI scope + §9.1 gate). Unplanned gaps fixed on `security/audit-hardening-2026-07-10`: auth-endpoint rate limiting + `trust proxy` (H1/M5), helmet security headers (M6), acting-role bound to the token claim not a header (M4), recovery-code lockout (M7), RLS-role boot guard (M2), admin-reset ADMIN gate (L1), non-local secret guard (L2), JWT alg pin (L5), mitigation-log validation (L7), server `npm audit fix` + client react-router bump. **Deliberately left for P5 (single-instance deploy):** DB TLS CA-pinning and Redis-backed rate-limit/replay caches. 386 unit / 202 client green at branch time; merged and built on by O6/O7 (full suite incl. integration green since).
...
| AI features (6) | AI service module | 🟡 **P4 build STARTED 2026-07-04** — playbook `docs/plans/p4-execution-plan.md` (O1–O9 + **3 gates: F2 / F-AI / F3**) + DoD `docs/test-specs.md` §P4/§P4.5. **O1–O7 ✅ + F2 ✅** (2026-07-04) + **F-AI ✅** (2026-07-16, live Gateway smoke). **⬅ NEXT: O8 — P4.5 part 1 (PLATFORM_OWNER + provisioning API), then the F3 gate**. Remaining: O8 → F3 → O9. Two client gaps are deliberate and tracked as backlog: AD-1's chip/ack still computes locally (O6 is server-only), and the HQ consistency panel is read-only (O7's dismiss/send-back endpoint has no UI). The nightly consistency job needs a Render cron + an elevated DB connection once P2's non-owner switch lands — see SESSION_LOG 2026-07-16. |
