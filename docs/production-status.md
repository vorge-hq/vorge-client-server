...
Last updated: 2026-07-10 · Branch: `main` (open: `security/audit-hardening-2026-07-10`, awaiting owner review). Brand: **Vorge** (rebranded from Vantage on 2026-06-02 — see SESSION_LOG).

> **2026-07-10 — security hardening pass (branch, not merged):** the AUDIT-BRIEF 5-agent review found the core posture strong (no SQLi, IDOR-proof tenancy, sound state machine/lockVersion, correct AI scope + §9.1 gate). Unplanned gaps fixed on `security/audit-hardening-2026-07-10`: auth-endpoint rate limiting + `trust proxy` (H1/M5), helmet security headers (M6), acting-role bound to the token claim not a header (M4), recovery-code lockout (M7), RLS-role boot guard (M2), admin-reset ADMIN gate (L1), non-local secret guard (L2), JWT alg pin (L5), mitigation-log validation (L7), server `npm audit fix` + client react-router bump. **Deliberately left for P5 (single-instance deploy):** DB TLS CA-pinning and Redis-backed rate-limit/replay caches. 386 unit / 202 client green; integration not re-run (no local DB). Awaiting owner review before merge.

> **2026-07-03:** The production push now runs on **`docs/roadmap.md` (P0–P5)** — that file is the execution checklist to tick; this file stays the product-state map. The 2026-06-04 strategic retrospective (originally authored at `docs/roadmap.md`) now lives at **`docs/strategic-roadmap.md`**. Locked: Supabase (managed Postgres only), Render (Docker web service), new non-demo Vercel project, Vercel AI Gateway + AI SDK (§9 deviation + write-API contract extension both signed off 2026-07-03 — `docs/decisions/2026-07-03-*`). P0 in progress. **2026-07-03 (later):** roadmap-P3 (write/section API) **server-side complete** — all content CRUD (assets/threats/links/evaluations/contributors/section-text) + withdraw/recall + Lead Author reassignment + mitigation-owner assignment, through one shared write-guard (six-case guards + optimistic concurrency + atomic audit), 250 unit / 107 integration green. Also fixed a read-your-writes gap in P2's `facilityScope` (commit-before-flush, decision recorded). P3 (g) client flip **in progress** — write seam (all endpoints) + narrative saves for §1/§8 (live w/ lockVersion in prod, 409 → reload copy; demo no-fetch); prod reads now hydrate the assessments **list** (`GET /api/assessments`, server-scoped) and the opened **workspace** assessment (fields + section text) via an adapter. Demo untouched. **2026-07-04:** content-entity **reads** flipped (assets/threats/links→matrix/evaluations hydrate from the bundle in prod); canonical JSONB field mapping recorded (`docs/decisions/2026-07-04-content-entity-field-mapping.md`). **2026-07-04 (later):** §2 Facility Info decided + wired and **ALL content-entity writes** flipped — Assets/Threats (CRUD), asset×threat-link toggle (`PUT /links`), Evaluations (PATCH on blur), Contributors (whole-list PUT) — through the prod↔demo seam with lockVersion sync + 409 reload banners; demo fires nothing. **P3 (g) client flip COMPLETE** (250 unit / 176 client / 109 integration green). The evaluation create-path gap was closed the same day: `setAssetThreatLink` auto-creates + echoes an empty evaluation when a pair is enabled, so freshly-ticked Section-6 cells persist. **P3 write/section API is now feature-complete end-to-end.** See SESSION_LOG + `docs/roadmap.md` P3.
...
| Field mode | Offline/field foundations | 🟡 **P6 SPECCED 2026-07-04, build after P4.5** — whole-assessment checkout + sync replay + PIN, decided in `docs/decisions/2026-07-04-offline-mode-architecture.md`; playbook `docs/plans/p6-offline-execution-plan.md` (O1–O6 + F4/F5 gates); DoD `docs/test-specs.md` §P6. Client stubs remain simulation-only until the P6 build. |
...
| AI features (6) | AI service module | 🟡 **P4 build STARTED 2026-07-04** — playbook `docs/plans/p4-execution-plan.md` (O1–O9 + 2 review gates) + DoD `docs/test-specs.md` §P4/§P4.5. **O1 Library CRUD ✅ + O2 AI foundation ✅ + F2 gate ✅ + O3 Semantic search ✅ + O4 Smart tagging ✅ + O5 Drafted summary ✅ — all 2026-07-04.** O2: `server/src/ai/` (`runAiCall`, gateway egress, prompt scoping), `aiBudgetService`, migrations `ai_call_log`/`ai_budgets`/`facility_entitlements` (RLS) + entitlement gating + `aiImportBoundary`. O3: `library_entries.embedding vector(1536)`, async post-commit embedding pipeline, `GET /api/library/search` (cosine, facility-scoped), reembed script, LibraryModal wired through the seam. O4: `tag_vocabulary`+`scenario_tags` (RLS), `suggest-tags`/`tags/confirm` endpoints (structured output, out-of-vocab discarded, separate suggested/confirmed audit), `ScenarioTags` chips through the seam. O5: `POST …/sections/:n/generate-draft` (n∈{1,8}; Author+non-Approved; `runAiCall({kind:'text'})`; AI original retained in `ai-draft-generated` audit next to the edited final), "Draft with AI" flow in §1/§8 through the seam. 386 unit / 172 integration / 202 client green. **⬅ NEXT: O6 anomaly detection server engine (`/model opus`).** AD-1 anomaly ack shipped 2026-05-29 (client-only); AD-2+ server engine = O6. |
