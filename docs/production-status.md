...
| AI features (6) | AI service module | 🟡 **P4 build STARTED 2026-07-04** — playbook `docs/plans/p4-execution-plan.md` (O1–O9 + 2 review gates) + DoD `docs/test-specs.md` §P4/§P4.5. **O1 Library CRUD ✅ + O2 AI foundation ✅ + F2 gate ✅ + O3 Semantic search ✅ — all 2026-07-04.** O2: `server/src/ai/` (`runAiCall`, gateway egress, prompt scoping), `aiBudgetService`, migrations `ai_call_log`/`ai_budgets`/`facility_entitlements` (RLS) + entitlement gating + `aiImportBoundary`. O3: `library_entries.embedding vector(1536)`, async post-commit embedding pipeline, `GET /api/library/search` (cosine, facility-scoped), reembed script, LibraryModal wired through the seam. 366 unit / 152 integration / 190 client green. **⬅ NEXT: O4 smart tagging (`/model opus`).** AD-1 anomaly ack shipped 2026-05-29 (client-only); AD-2+ server engine = O6. |
