...
| Phase 2 — Tenant isolation hardening | 🟡 **In progress (2026-07-03)** | Integration harness + repo isolation + SQL-scoped list landed. **Cross-tenant ROUTE matrix now green** (supertest + real DB: cross-tenant GET/list/workflow/mitigation-log → 404, wrong-role → 403, unauth → 401, HQ §17.5 portfolio). Outstanding: route-guard introspection test, `requireFacilityAccess` wiring decision, Supabase RLS policies + non-owner app role. |
