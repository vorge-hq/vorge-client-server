...
| AI features (6) | AI service module | 🟡 **P4 build STARTED 2026-07-04** — playbook `docs/plans/p4-execution-plan.md` (O1–O9 + 2 review gates) + DoD `docs/test-specs.md` §P4/§P4.5. **O1 Library CRUD ✅ + O2 AI foundation ✅ DONE.** O2: `server/src/ai/` (`runAiCall`, gateway egress, prompt scoping), `aiBudgetService` (ceiling math), migrations `ai_call_log`/`ai_budgets`/`facility_entitlements` (RLS) + read-time entitlement gating + `aiImportBoundary` scan — 336 unit / 143 integration green. **⬅ NOW: STOP for F2 Fable gate before O3 (semantic search).** AD-1 anomaly ack shipped 2026-05-29 (client-only); AD-2+ server engine = O6. |
