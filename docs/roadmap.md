...
- [P1/S] **Evaluation create endpoint (server gap from P3 (g))** — evaluations expose only `PATCH /evaluations/:id`; there is no create path and enabling a link doesn't seed one, so a Section-6 evaluation newly created in prod (ticking a fresh matrix cell) cannot be persisted. Options: `POST /assessments/:id/evaluations`, or auto-create an empty evaluation row inside `setAssetThreatLink` when a link is enabled (client then PATCHes it — needs the created id echoed back). Client already guards (`persistEvaluation` skips non-UUID stub ids). — **NEEDS DECISION** (which approach; small server slice)
